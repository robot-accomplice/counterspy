<h1>
  <picture>
    ...
    <img alt="" width="40" height="40" align="absmiddle" src="docs/assets/counterspy-light.svg">
  </picture>
  CounterSpy <sub>counter-surveillance for your own Mac</sub>
</h1>
...
**Exfiltration monitor (`console` → Tab):** a live, per-app "egress top" showing outbound rate, a
trend sparkline, destinations (with a `▣` TLS / `□` cleartext glyph), and an inferred exfil
concern, all through the code-sign trust + provenance lens.
...
and extract. Builds aren't notarized yet (signing is imminent), so `curl | sh` and Homebrew sidestep
...
- **Correlated evidence.** A listener spawned by a LaunchAgent from an unsigned binary
  that also holds Input Monitoring is one story told four ways, and that correlation is how
...
- **Per-app rollup:** every PID, port, and protocol of an app collapses into one expandable row
  showing outbound rate, a trend sparkline, destinations, and a cadence (one-off / bursty / steady /
...
- **Zoom (`z`):** a btm-style dashboard for one app, showing a braille throughput graph (one line per
  PID, or per destination via `g`), a selectable table with each PID's rate + %-of-group share, its
...
- **Inspect (`i`):** capture and inspect one flow (native `/dev/bpf`, needs sudo) with an **honest
  ...
  payload), and plaintext is shown masked until you press `v`. Cleartext HTTP is decoded (dechunked
  and gzip/deflate-decompressed) so a compressed body reads as text, not a hexdump.
...
fired, a score *band*, the category, code-sign status, and a path *class*, with no raw paths,
...
Kernel/firmware implants, SIP-protected components, or signed supply-chain malware. See the
[threat model](docs/threat-model.md) for the full scope and non-goals.
